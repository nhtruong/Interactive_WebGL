$.widget("custom.dancingSlider", {
    ...
    _create: function(){
        ...
        this.element.addClass('ds_wrapper')
            .prepend($("<div style='clear:both;height:10px;'>&nbsp</div>"));
        this.label = $("<div class='ds_label'></div>").html(this.options.label);
        this.text = $("<input class='ds_text' type='text'/>").change(function(event,ui){
            self.value($(this).val());
        }).keydown(function(event){
            if(event.keyCode === 38 || event.keyCode === 40) {
                event.preventDefault();
                var step = (self.options.max - self.options.min) / 100;
    ...
    updateMeter: function(){
        var value = this.options.step < 1 
            ? this.options.value.toFixed(this.options.precision) 
            : Math.round(this.options.value);
        this.text.val(value);
    ...
    setupDanceFloor: function(arguments){
        ...
        this.floor_range = $("<div class='ds_slider'></div>").slider({
            ...
            max: this.options.max ,
            step: this.options.step,
            values: arguments.values === undefined ? [this.options.min, this.options.max] : arguments.values,
            slide: function(event, ui){
        ...
        this.floor
            ...
            .append($("<div></div>").append(this.floor_range))
            .append($("<div style='clear:both;height:0px;'>&nbsp</div>"))
            .append($("<div></div>").append(this.floor_center))
            .append($("<div style='clear:both;height:0px;'>&nbsp</div>"))
            .append($("<div></div>").append(this.floor_flip))
            .append($("<div style='clear:both;height:10px;'>&nbsp</div>"))
            .append($("<div></div>").append(this.floor_speed))
            .append($("<div style='clear:both;height:10px;'>&nbsp</div>"));
        ...
        this.floor_enabled = $("<button class='ds_toggle'/>")
                .button({text: false,label:"pause",icons: {primary: "ui-icon-pause"}})
                .click(function () {
                    var options;
